...
class PoseDetector(private val context: Context) : Closeable {
    ...
    suspend fun run(source: Bitmap, backend: ComputeBackend, sourceCount: Int): PreviewRenderResult =
        withContext(dispatcher) {
            // Each backend owns one long-lived ORT session so benchmark numbers do not include
            // repeated session creation or provider initialization cost.
            val session = sessions.getOrPut(backend) {
                OnnxRuntimeBackends.createSession(
                    context = context,
            ...
            val totalMs = measureNanoTime {
                ...
                inferenceMs = measureStepMs {
                    ortResult = session.session.run(
                        mapOf(INPUT_NAME to checkNotNull(inputTensor))
                    )
                }

                postprocessMs = measureStepMs {
                    checkNotNull(ortResult).use { result ->
                        val simccX = (result.get(OUTPUT_X_NAME).orElseThrow() as OnnxTensor).getFloatBuffer()
                        val simccY = (result.get(OUTPUT_Y_NAME).orElseThrow() as OnnxTensor).getFloatBuffer()
                        decodedPose = decodeKeypoints(simccX, simccY, preprocessOutput)
                    }
                }
            } / 1_000_000.0
    ...
    private fun decodeKeypoints(
        simccX: java.nio.FloatBuffer,
        simccY: java.nio.FloatBuffer,
        ...
        val xValues = FloatArray(KEYPOINT_COUNT * SIMCC_X_DIM)
        val yValues = FloatArray(KEYPOINT_COUNT * SIMCC_Y_DIM)
        simccX.rewind()
        simccY.rewind()
        simccX.get(xValues)
        simccY.get(yValues)
    ...
    private fun drawPose(
        ...
        canvas.drawText(
            "RTMPose ONNX Runtime (${backend.displayName})",
            result.width * 0.05f,
            result.height * 0.08f,
            textPaint
        )
    ...
    companion object {
        private const val MODEL_ASSET_NAME = "models/rtmpose_t_body7_256x192.onnx"
        private const val INPUT_NAME = "input"
        private const val OUTPUT_X_NAME = "simcc_x"
        private const val OUTPUT_Y_NAME = "simcc_y"
